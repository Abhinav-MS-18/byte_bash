...
const Register = () => {
  ...
  const handlePinChange = (index, value, setPinFunc, pinArray) => {
    if (/^[0-9]?$/.test(value)) {
      ...
      if (value !== "" && index < 5) {
        const nextInput =
          pinArray === pin ? `pin-${index + 1}` : `confirmPin-${index + 1}`;
  ...
  return (
    <div className="flex flex-col justify-center ml-36 items-start min-h-screen text-white">
      ...
      <h1 className="text-3xl z-40 font-bold mb-4 text-green-400">
        Register as New User
      </h1>

      <form
        className="w-full max-w-lg flex flex-col gap-5 mt-5"
        onSubmit={
          step === 3
            ? handleRegister
            : (e) => {
      ...
      >
          ...
          <>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-2">
                {pin.map((digit, index) => (
                  <TextInput key={index} id={`pin-${index}`} value={digit} type="password" maxLength={1} className="w-10 text-center"
                    onChange={(e) =>
                      handlePinChange(index, e.target.value, setPin, pin)
                    }
                    onKeyDown={(e) =>
                      e.key === "Backspace" && handlePinDelete(index, setPin, pin)
                    }
                    required
                  />
                ))}
        ...
        <div className="flex justify-between">
          <Button
            color="gray"
            onClick={() => {
              step === 1 ? navigate("/") : handleBack();
            }} disabled={loading}>
            Back
          </Button>
          {step === 3 ? (
            <Button type="submit" isProcessing={loading} gradientDuoTone="greenToBlue">
              Register
            </Button>
          ) : (
